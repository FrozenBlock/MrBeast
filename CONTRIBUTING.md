...
By contributing to MrBeast, you agree with the [Developer Certificate of Origin (DCO)][DCO].
...
Use American English to keep consistency across MrBeast.
...
It allows to separate the actual name of the variable and the namespace, `mrBeast`.

Fields marked as `@Unique` must be prefixed with `mrBeast$` or `MRBEAST$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

MrBeast is licensed under [LGPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "MrBeast license file"
